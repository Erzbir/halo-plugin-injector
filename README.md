...
> 如果没有页面路径条件，或者写成“页面路径 / 模板 ID 二选一”这类规则，插件仍然会工作，
> 但元素 `ID` / `CSS 选择器` 模式会在更多 HTML 页面上参与处理，配置页也会显示性能警告。
...
- 若元素 `ID` / `CSS 选择器` 规则未先使用页面路径缩小范围，插件仍会继续工作，但会带来一些额外处理开销
