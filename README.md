...
>
> 这两种模式的匹配规则里，必须先有“页面路径”条件，
> 这样插件才能先根据访问路径判断“这次请求需不需要继续处理整页 HTML”，再用模板 ID 继续缩小范围。
> 所以模板 ID 只能作为额外限制条件使用，不能单独决定是否触发 `id` / `selector` 注入。
...
- 匹配内容（`value`）是否为空
- `REGEX` 是否可正常编译
- 元素 `ID` / `CSS 选择器` 模式下，匹配规则里是否先包含页面路径条件
