...
>
> 这两种模式的匹配规则还必须满足“可按路径预筛”：
> 也就是先靠路径判断“这次请求值不值得进入整页 HTML 处理”，再由模板 ID 继续缩小范围。
> 因此模板 ID 只能作为已命中路径分支上的附加条件，不能单独决定是否触发 DOM 注入。
...
> 因此 `remove` 模式下无需关联代码块；保存时会自动清空关联，后端也会拒绝携带 `snippetIds` 的脏数据。
...
每条注入规则都使用一棵 `matchRule` 规则树描述“在什么情况下生效”。
...
- `value` 是否为空
- `REGEX` 是否可正常编译
- `ID` / `SELECTOR` 是否满足路径预筛约束
- `REMOVE` 是否错误携带了 `snippetIds`
