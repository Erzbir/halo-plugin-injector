...
当前标签页、选中项和新建弹窗状态会同步到地址栏，便于刷新后恢复或直接分享链接，例如：
...
> 编辑时会保留你之前勾选的状态，方便切回其它插入位置继续使用；
> 真正保存为 `remove` 时，系统才会自动关闭这一项，后端也会拒绝错误数据（内部字段名为 `wrapMarker`）。
...
- 当前规则使用的是“简单模式”还是“高级模式（JSON）”，以及当前 JSON 草稿，会在本地记住；刷新页面后会尽量恢复
